...
app.use(express.urlencoded({ extended: true })); // スライド[cite: 285]

// データ（スライドの station2 に相当）
let macItems = [
...
// 一覧表示 (Read) - スライド[cite: 285]参照
app.get("/mac", (req, res) => {
  res.render('mac', {data: macItems} );
});

app.get("/sukiya", (req, res) => {
  res.render('sukiya', {data: sukiyaItems} ); // ファイル名もデータもsukiyaに変更
});
...
// 新規登録画面へ (Create) - スライド[cite: 285]参照
...
app.post("/mac", (req, res) => {
  ...
  // スライド通り console.log して render で返す
...
app.post("/sukiya", (req, res) => {
  ...
  const price = req.body.price;
  const category = req.body.category;
  const calorie = req.body.calorie; // 全角スペース注意（修正済み）
